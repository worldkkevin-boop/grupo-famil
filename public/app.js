...
async function abrirSuperAdmin() {
  $('modal-superadmin').classList.add('active');
  carregarGruposSaaS();
}

async function carregarGruposSaaS() {
  const lista = $('lista-grupos-saas');
  lista.innerHTML = '<p style="color:var(--text-soft)">Carregando...</p>';
  try {
    ...
    lista.innerHTML = '';
    if (grupos.length === 0) {
      lista.innerHTML = '<p style="color:var(--text-soft)">Nenhum grupo encontrado.</p>';
      return;
    }

    grupos.forEach(g => {
      const div = document.createElement('div');
      div.style.cssText = 'background:var(--surface); border:1px solid var(--border); padding:16px; border-radius:12px; display:flex; justify-content:space-between; align-items:center;';
      div.innerHTML = `
        <div>
          <h4 style="color:#fff; margin-bottom:4px;">${g.nome} (ID: ${g.id})</h4>
          <p style="color:var(--text-soft); font-size:0.85rem; margin:0;">
            Dono: ${g.dono_email} • Membros: ${g.qte_membros} <br>
            Pix: ${g.pix_key} (${g.pix_name})
          </p>
        </div>
        <button onclick="excluirGrupoSaaS(${g.id})" style="background:rgba(248,113,113,0.1); border:1px solid var(--danger); color:var(--danger); padding:8px; border-radius:8px; cursor:pointer;" title="Deletar Grupo de Teste">
          🗑️
        </button>
      `;
    ...
    lista.innerHTML = `<p style="color:var(--danger)">${err.message}</p>`;
  }
}

async function excluirGrupoSaaS(id) {
  if (id === 1) {
    alert("O Grupo 1 é o seu grupo original e não pode ser apagado por segurança!");
    return;
  }
  if (!confirm("Tem certeza absoluta? Isso apagará o grupo, membros, assinaturas e pagamentos DESTE cliente!")) return;
  try {
